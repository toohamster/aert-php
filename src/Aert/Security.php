...
class Security {
	...
	static function xss_clean($str, $is_image = FALSE)
	{
		...
		$str = preg_replace_callback("/[a-z]+=([\'\"]).*?\\1/si", array(__CLASS__, '_convert_attribute'), $str);

		$str = preg_replace_callback("/<\w+.*?(?=>|<|$)/si", array(__CLASS__, '_decode_entity'), $str);
		...
		foreach ($words as $word)
		{
			...
			$str = preg_replace_callback('#('.substr($temp, 0, -3).')(\W)#is', array(__CLASS__, '_compact_exploded_words'), $str);
		}
		...
		do
		{
			...
			{
				$str = preg_replace_callback("#<a\s+([^>]*?)(>|$)#si", array(__CLASS__, '_js_link_removal'), $str);
			}

			if (preg_match("/<img/i", $str))
			{
				$str = preg_replace_callback("#<img\s+([^>]*?)(\s?/?>|$)#si", array(__CLASS__, '_js_img_removal'), $str);
			}
		...
		$str = preg_replace_callback('#<(/*\s*)('.$naughty.')([^><]*)([><]*)#is', array(__CLASS__, '_sanitize_naughty_html'), $str);
	...
	{
		return self::entity_decode($match[0]);
	}
	 ...
	 * Do Never Allowed
	 *
	 * A utility static function for xss_clean()
	 *
	 * @param 	string
	 * @return 	string
	 */
