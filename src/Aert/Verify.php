...
class Aert_Verify 
{
    ...
    private function __construct(){$this->_locale = localeconv();}
     ...
     * 是否是字母加数字
     */
    function is_alnum($value){return ctype_alnum($value);}

    /**
     * 是否是字母
     */
    function is_alpha($value) {   return ctype_alpha($value); }

    /**
     * 是否是字母、数字加下划线
     */
    function is_alnumu($value){return preg_match('/[^a-zA-Z0-9_]/', $value) == 0;}
    
    function is_chinese($value){return preg_match( "/^[\x80-\xff]+/",$value,$match) && ($match[0] == $value);}

    /**
     * 是否是控制字符
     */
    function is_cntrl($value){return ctype_cntrl($value);}

    /**
     * 是否是数字字符
     */
    function is_digits($value){return ctype_digit($value);}
	    
   /**
     * 是否是数字(包括小数点的浮点型数字)
     */
    function is_numeric($value){return is_numeric($value);}
    
    /**
     * 是否是可见的字符
     */
    function is_graph($value){return ctype_graph($value);}

    /**
     * 是否是全小写
     */
    function is_lower($value){return ctype_lower($value);}

    /**
     * 是否是可打印的字符
     */
    function is_print($value){return ctype_print($value);}

    /**
     * 是否是标点符号
     */
    function is_punct($value){return ctype_punct($value);}

    /**
     * 是否是空白字符
     */
    function is_whitespace($value){return ctype_space($value);}

    /**
     * 是否是全大写
     */
    function is_upper($value){return ctype_upper($value);}

    /**
     * 是否是十六进制数
     */
    function is_xdigits($value){return ctype_xdigit($value);}

    /**
     * 是否是 ASCII 字符
     */
    function is_ascii($value){return preg_match('/[^\x20-\x7f]/', $value) == 0;}

    /**
     * 是否是电子邮件地址
     */
    function is_email($value){return preg_match('/^[a-z0-9]+[\._\-\+]?([a-z0-9])+@([a-z0-9]+[-a-z0-9]*\.)+[a-z0-9]+$/i', $value);}
     ...
     */
    function is_ipv4($value){$test = @ip2long($value);return $test !== - 1 && $test !== false;}

    // 是否是八进制数值
    function is_octal($value){return preg_match('/0[0-7]+/', $value);}

    /**
     * 是否是二进制数值
     */
    function is_binary($value){return preg_match('/[01]+/', $value);}

    /**
     * 是否是 Internet 域名
     */
    function is_domain($value){return preg_match('/[a-z0-9\.]+/i', $value);}
